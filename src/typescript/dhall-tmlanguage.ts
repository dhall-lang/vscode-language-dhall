...
const url = `(?:${scheme}:\\/${authority}${directory}${file}(?:\\\\?${query})?(?:#${fragment})?)`;
...
const tmLanguage: TmLanguage = {
  ...
  repository: {
    keywords: {
      patterns: [
        ...
        },
        {
          ...
          match:
            "\\b(Natural/fold|Natural/build|Natural/isZero|Natural/even|Natural/odd|Natural/toInteger|Natural/show|Integer/toDouble|Integer/show|Double|Double/show|List/build|List/fold|List/length|List/head|List/last|List/indexed|List/reverse|Optional/fold|Optional/build)\\b",
        },
        {
      ...
      patterns: [
        {
          ...
          begin: "\\$\\{",
          beginCaptures: {
            "0": { name: "punctuation.section.curly.begin.dhall" },
          },
          end: "\\}",
    ...
    labelBind: {
      // * defines constant value in let expression
  ...
  $schema:
    "https://raw.githubusercontent.com/Septh/tmlanguage/master/tmLanguage.schema.json",
...
let schema = fs
  .readFileSync("./src/typescript/tmlanguage.schema.json")
  .toString();
